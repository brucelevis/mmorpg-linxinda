...
public class PlayerInfoEncoder {
    ...
    public String encode(PlayerDb db) {
        ...
        String msg = "登录成功|玩家名：%s，Hp: %s，Mp：%s，Exp: %s，职业：%s，当前所在场景：%s  (playerUid：%s)";
        return String.format(msg, db.getName(), db.getHp(), db.getMp(), db.getExp(), factionName, sceneName, db.getUid());
    }
}
